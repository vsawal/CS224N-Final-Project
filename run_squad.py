...
def main():
    ...
    parser.add_argument("--squad_model", default='bert_base', type=str, required=True,
    ...
    if os.path.isfile(model_file) and os.path.isfile(config_file):
        model = create_model(args, device, config_file, model_file)
    else:
    ...
    tbx = SummaryWriter(args.output_dir)
    global_step = 0
    if args.do_train:
        ...
        saver = utils.CheckpointSaver(args.output_dir,
                                      max_checkpoints=3,
                                      metric_name='F1',
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                if step != 0 and step % 500==0:
                    model.eval()
                    allresutls = evaluate(model, device, eval_examples, eval_features, eval_data, args.output_dir,ep,step,
                                             predict_batch_size=8)
                    out_eval=e.main(eval_file, args.output_dir + '/'+str(ep)+"_"+str(step)+"predictions.json")
